...
#@njit(parallel = True)
def detect_damaged_pixels(frames, plot=True, consecutive_threshold=50, mse_threshold=100, ssim_threshold = 0.1, brightness_threshold = 170):
    """
    ...
    for i in range(num_frames):
        ...
        filtered_damaged_pixels = remove_bright_regions(background, threshold, height, width, filtered_damaged_pixels, max_cluster_size)
    ...
    for row in prange(height):
        for col in prange(width):
            ...
            if frame[row, col] > threshold:
                ...
                kernel = frame[max(row - 100, 0) : min(row + 200, height), max(col - 100, 0) : min(col + 200, width)]
                kernel_mean = np.mean(kernel)
                
                if frame[row, col] > (0.25 * kernel_mean):
...
#@njit(parallel = True)
def remove_bright_regions(background, brightness_threshold, height, width, filtered_damaged_pixels, max_cluster_size):
    """
    ...
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(bright_background_mask, connectivity = 8)
...
def visualize_damaged_pixels(frame, damaged_pixels, frame_index, estimate_count = None, bright_threshold = 170):
    """
...
def create_clustered_test_video(num_frames = 100, width = 928, height = 576, cluster_count = 50, cluster_size_range = (10, 20), background_intensity = 0):
    """
    ...
    for _ in range(num_frames):
        ...
        for _ in range(cluster_count):
            ...
            while overlap_detected and tries < 10:
                ...
                overlap_detected = any(abs(cluster_center_x - cx) < 20 and abs(cluster_center_y - cy) < 20 for cx, cy in cluster_centers)
